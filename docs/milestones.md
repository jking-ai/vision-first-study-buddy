...
- [ ] `app/config.py` loads settings from environment variables: `GCP_PROJECT_ID`, `GCP_REGION` (default: `us-central1`), `GEMINI_MODEL` (default: `gemini-1.5-flash`), `FIREBASE_STORAGE_BUCKET`
...
- [ ] A standalone test script (`scripts/test_gemini_multimodal.py`) sends a handwritten notes image to Gemini 1.5 Flash and prints the extracted text, confirming multimodal authentication works
...
**Acceptance criteria:**
- [ ] React 19 project initialized with Vite and MUI: `npm create vite@latest frontend -- --template react`
- [ ] MUI ThemeProvider configured with custom theme supporting light and dark modes
- [ ] `src/api/client.js` exports an API client with methods for all backend endpoints
- [ ] API base URL is configurable via `VITE_API_URL` environment variable (defaults to `http://localhost:8000`)
- [ ] `TopNav.jsx` renders an app bar with navigation links and a dark mode toggle
- [ ] `MaterialUpload.jsx` renders a drag-and-drop zone and file picker supporting images, PDFs, and epubs
- [ ] Upload progress is shown with a progress bar
- [ ] `MaterialList.jsx` displays uploaded materials as cards with thumbnails (for images) and file type icons (for documents)

#### 3.2 Frontend -- Camera Capture

**Acceptance criteria:**
- [ ] `CameraCapture.jsx` integrates with the device camera using the MediaDevices API (`navigator.mediaDevices.getUserMedia`)
- [ ] Shows a live camera preview with a capture button
- [ ] On capture, the photo is previewed and the user can accept or retake
- [ ] Accepted photos are automatically uploaded via the materials upload endpoint
- [ ] Falls back gracefully on devices without camera access (shows file picker instead)
- [ ] Works on iOS Safari and Android Chrome

#### 3.3 Frontend -- Study Guide and Quiz Views

**Acceptance criteria:**
- [ ] `StudyGuidePage.jsx` allows selecting materials and clicking "Generate Study Guide"
- [ ] Loading state shows a skeleton or spinner with a message ("Analyzing your materials...")
- [ ] `StudyGuideView.jsx` renders the study guide with collapsible sections, highlighted key terms, and a clean reading layout
- [ ] `QuizPage.jsx` allows selecting materials, configuring quiz options (question count, difficulty), and generating a quiz
- [ ] `QuizView.jsx` renders questions as interactive cards -- radio buttons for multiple choice, text input for short answer
- [ ] A submit button grades the quiz and shows results inline with color-coded correct/incorrect indicators and explanations
...
**Acceptance criteria:**
- [ ] Dark mode toggle in TopNav switches the entire app between light and dark themes using MUI's `useColorMode`
- [ ] Responsive layout works on desktop (1200px+), tablet (768px+), and mobile (375px+)
- [ ] All loading states show appropriate feedback (skeletons, spinners, progress bars)
- [ ] All error states display user-friendly messages with retry options
- [ ] Empty states (no materials uploaded, no study guides generated) show helpful onboarding messages
- [ ] `README.md` is updated with live demo links, screenshots, setup instructions, and a "Technical Highlights" section
- [ ] All planning docs (architecture.md, api-contracts.md, milestones.md) remain accurate and reflect the final implementation
